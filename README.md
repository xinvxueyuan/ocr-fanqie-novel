...
- **FR7/FR8 超时/重试**：超时未响应或连续识别失败达到上限，转入"待管理员决策"并通知管理员
- **管理决策超时**：通知管理员后超过 16 小时（可配置）未处理，自动在群内通报并移出成员
...
4. 否则（他人书评 / 缺少书名作者 / 白名单不命中 / 超时未发图 / 识别失败达上限）→ 转入**待管理员决策**并私信通知管理员，管理员用 `/kick` 或 `/keep` 决策
5. 管理员超过 `FANQIE_ADMIN_DECISION_TIMEOUT`（默认 16 小时）未处理 → 机器人在群内通报并移出该成员
